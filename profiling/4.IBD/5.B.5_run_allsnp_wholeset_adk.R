...
get_inputdf <- function(mygeno=a2, phenopwd){
  ...
  for(i in 1:7){ #traits
    ...
    tem2 <- data.frame(
        pi = 0.995, geno = mygeno[idx],
        pheno = paste0(phenopwd, ti[i], "_MPH.txt"),
        chainLength=41000, burnin=1000, varGenotypic=gen[i], varResidual=res[i]
    ) 
    inputdf <- rbind(inputdf, tem, tem2)
  }
...
adk2 <- list.files(path="/home/jolyang/Documents/Github/GERP-diallel/largedata/sgeno", 
                 pattern="gs.newbin$", full.names=TRUE)
adk <- adk2[grep("stime0", adk2)]


### adk for both perse and MPH
inputdf1 <- get_inputdf(mygeno=adk, phenopwd)
inputdf1$out <- paste(gsub(".*\\/|.txt", "", inputdf1$pheno), gsub(".*\\/|.gs.newbin", "", inputdf1$geno), sep="_" )
